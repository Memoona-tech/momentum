...
function scheduleLabel(habit: Habit): string {
  ...
  if (habit.scheduleType === "weekly") {
    return habit.scheduleDays.map((d) => WEEKDAY_LABELS[d]).join(", ") || "No days set";
  }
  return habit.scheduleDays.map((d) => `${d}${nth(d)}`).join(", ") || "No days set";
}
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2.5">
        {visible.length === 0 && (
          <p className="text-void-400 text-sm">{showArchived ? "No archived habits." : "No habits yet — create your first one."}</p>
        )}
        {visible.map((h) => (
          <div key={h.id} className="flex items-center gap-3 rounded-xl border border-void-600 bg-void-900/60 p-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-medium text-sm text-parchment">{h.name}</p>
                <CategoryTag category={categories.find((c) => c.id === h.categoryId)} />
              </div>
              {h.description && <p className="text-xs text-void-400 mt-0.5">{h.description}</p>}
              <p className="text-[11px] text-void-400 mt-1 font-mono">
                {scheduleLabel(h)}
                {h.targetTime ? ` · ${h.targetTime}` : ""}
                {h.targetValue ? ` · ${h.targetValue} ${h.targetUnit ?? ""}` : ""}
              </p>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => setEditing(h)} className="p-2 text-void-400 hover:text-gold-400 transition-colors" aria-label="Edit">
              ...
              >
                {h.archived ? <ArchiveRestore size={16} strokeWidth={1.75} /> : <Archive size={16} strokeWidth={1.75} />}
              </button>
              <button onClick={() => handleDelete(h)} className="p-2 text-void-400 hover:text-red-400 transition-colors" aria-label="Delete">
